...
type
  ...
  TAfterRecive = function(AcceptedSession:pointer;Data:pointer;var DataSize:integer): string;
...
FInputBuffer:=TList.Create;
FOutputBuffer:=TList.Create;
FIsActiveSession:=FIsActiveSession;
...
if not FIsActiveSession then
  ...
  begin
   ...
   TmpRecivedSize:=ReciveInternal(TmpBuffer);
   if Assigned(FAfterRecive) then
      FAfterRecive(TmpCETreadingServ,TmpBuffer,TmpRecivedSize);
   If FEnableInternalInBuffer then
     Begin
       ...
       begin
         if FWritePos=FInputBuffer.Count then
           FWritePos:=0;
...
function TCETCPTThreadingSocket.GetAcceptedSession(SockID:Double):TCETCPSocket;
var
SocketIndex:Integer=-1;
begin
SocketIndex:=GetSocketIndexById(SockID);
...
function TCETCPTThreadingSocket.GetAcceptedSession(SockTag:string):TCETCPSocket;
var
SocketIndex:Integer=-1;
begin
SocketIndex:=GetSocketIndexByTag(SockTag);
